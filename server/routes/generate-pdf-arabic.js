...
const generateArabicPDF = async (req, res) => {
  ...
  try {
    ...
    // Generate HTML content based on quotation data with enhanced styling
    const htmlContent = `
      <!DOCTYPE html>
          ...
          <style>
            ...
            * {
            ...
            .total-row, .info-row {
              ...
              align-items: center;
              padding: 8px 0;
              border-bottom: 1px solid #15921b;
              font-size: 12px;
            }

            .total-row:last-child {
              border-bottom: none;
              border-top: 2px solid #15921b;
              padding-top: 12px;
              font-weight: bold;
              font-size: 14px;
              color: #2c5d5a;
            ...
            .bank-info-item:nth-child(3) .bank-info-value {
              letter-spacing: 1px;
              white-space: nowrap;       /* Prevent wrapping */
              display: inline-block;     /* Keep box model consistent */
            }
        ...
        <body>
          <div class="document">
            ...
            <div class="title-section">
              <h1>عرض سعر</h1>
              <div class="quotation-number">رقم عرض السعر: ${quotation.number || 'غير محدد'}</div>
              <div class="customer-name">لـ: ${quotation.customerName || 'غير محدد'}</div>
            </div>
            ...
            <div class="info-grid">
              <div class="info-section">
                ...
                <p><strong>تاريخ الإصدار:</strong> ${new Date(quotation.issueDate || '').toLocaleDateString('ar-SA', { 
                  year: 'numeric', 
                  month: 'long', 
                  day: 'numeric' 
                }) || 'غير محدد'}</p>
                <p><strong>تاريخ الانتهاء:</strong> ${new Date(quotation.expiryDate || '').toLocaleDateString('ar-SA', { 
                  year: 'numeric', 
                  month: 'long', 
                  day: 'numeric' 
                }) || 'غير محدد'}</p>
              ...
              <div class="info-section">
                ...
                <p><strong>العميل:</strong> ${quotation.customerName || 'غير محدد'}</p>
                <p><strong>الحالة:</strong> في انتظار الموافقة</p>
              </div>
            </div>
              ...
              <table class="items-table">
                <thead>
                  <tr>
                    ...
                    <th style="width: 80px;">كود المنتج</th>
                    <th style="width: 300px;">الوصف</th>
                    <th style="width: 60px;">الكمية</th>
                    <th style="width: 80px;">السعر قبل الضريبة</th>
            ...
            <div class="totals-section">
              ...
              <div class="info-box">
                <div class="info-row">
                  ...
                  <span class="info-value">${quotation.validity || 'أسبوعين'}</span>
                </div>
                <div class="info-row">
                  <span class="info-label">مكان التسليم:</span>
                  <span class="info-value">${quotation.deliveryLocation || 'تبوك'}</span>
                </div>
                <div class="info-row">
                  <span class="info-label">فترة التسليم:</span>
                  <span class="info-value">${quotation.deliveryPeriod || 'أسبوع'}</span>
                </div>
                <div class="info-row">
                  <span class="info-label">الضمان:</span>
                  <span class="info-value">${quotation.warranty || 'ضمان الشركة المصنعة'}</span>
                </div>
              </div>
              ...
              <div class="totals-box">
                <div class="total-row">
                  <span class="total-label">الاجمالي الفرعي قبل الضريبة:</span>
                  <span class="total-amount">${formatCurrency(quotation.totalAmount)}</span>
                </div>
                <div class="total-row">
                  <span class="total-label">الخصم:</span>
                  <span class="total-amount">${formatCurrency(quotation.discount_rate || 0)}</span>
                </div>
                <div class="total-row">
                  <span class="total-label">قيمة الضريبة (15%):</span>
                  <span class="total-amount">${formatCurrency(quotation.vatAmount || 0)}</span>
                </div>
                <div class="total-row">
                  <span class="total-label">الاجمالي شامل الضريبة:</span>
                  <span class="total-amount">${formatCurrency((quotation.totalAmount || 0) + (quotation.vatAmount || 0))}</span>
                </div>
              </div>
            </div>
            ...
            <div class="bank-info-section">
              ...
              <div class="bank-info-message">
                <p>نأمل أن يلبي هذا العرض متطلباتكم ونتطلع إلى خدمة طلبكم الكريم. في الوقت نفسه، إذا كنتم بحاجة إلى أي توضيح أو مساعدة إضافية، فلا تترددوا في الاتصال بنا - سنكون سعداء لمساعدتكم في أي وقت.</p>
              </div>
              ...
              <div class="bank-info-content">
                <!-- Bank Info (Right for Arabic) -->
                <div class="bank-info-right">
                  <div class="bank-info-title-2">مع أطيب التحيات</div>
                  <div class="personal-info">
                    <p class="name">محمد شعبان</p>
                    <p class="title">مهندس شبكات وأنظمة</p>
                    <p>+966553373133</p>
                ...
                <!-- Bank Info (Left for Arabic) -->
                <div class="bank-info-left">
                  <div class="bank-info-title">معلومات البنك</div>
                    <div class="bank-info-item">
                      <span class="bank-info-label">اسم البنك:</span>
                      <span class="bank-info-value">${user.bank_name || 'غير محدد'}</span>
                    </div>
                    <div class="bank-info-item">
                      <span class="bank-info-label">الايبان:</span>
                      <span class="bank-info-value">${toArabicDigits(user.iban || 'غير محدد')}</span>
                    </div>
                    <div class="bank-info-item">
                      <span class="bank-info-label">اسم المستفيد:</span>
                      <span class="bank-info-value">شركة المعايير العصرية لتقنية المعلومات</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="terms-notes-section">
              ...
              <div class="content-box">
                ...
                ` : '<p>لا توجد شروط وأحكام محددة.</p>'}
              </div>

              <div class="section-title">ملاحظات إضافية</div>
              <div class="content-box">
                ${quotation.notes ? `<p>${quotation.notes}</p>` : '<p>لا توجد ملاحظات إضافية.</p>'}
              </div>
            </div>
            ...
            <div class="footer">
              <p>
                <strong>شركة المعايير العصرية لتقنية المعلومات</strong><br>
                العنوان: تبوك - شارع الملك فهد - حي السليمانية | 
                رقم الجوال: 0553373133 | 
                الإيميل: info@msit.com.sa
              </p>
            </div>
          </div>
        </body>
    ...
    await page.pdf({
      path: `Quotation-${quotation.number}.pdf`,
      format: 'A4',
    ...
    res.download(`Quotation-${quotation.number}.pdf`, () => {
      // Clean up the file after sending
      fs.unlinkSync(`Quotation-${quotation.number}.pdf`);
    });
  } catch (error) {
...
module.exports = { generateArabicPDF };